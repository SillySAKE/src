...
import purity.purpp.Client;
import purity.purpp.event.events.world.EventPostUpdate;
import purity.purpp.event.events.world.EventPreUpdate;
import purity.purpp.mods.Command;
import purity.purpp.mods.Module;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
    	//Setting up our EventPreUpdate
    	EventPreUpdate e = new EventPreUpdate(this.getEntityBoundingBox().minY,
				this.rotationYaw, this.rotationPitch, this.onGround);
    	
    	Client.instance.getEventBus().post(e);
    	
    	if(e.isCancelled()) {
    		Client.instance.getEventBus().post(new EventPostUpdate());
    		return;
    	}
    ...
    public void sendChatMessage(String p_71165_1_) {
    	if (p_71165_1_.startsWith(Client.prefix)) {

    		String[] command = p_71165_1_.split(" ");
    		
    		for (Entry<String[], Consumer<Command>> entry : Module.commands.entrySet()) {
    			for (String part : entry.getKey()) {
    				if (part.equalsIgnoreCase(command[0].replace(Client.prefix, ""))) {
    					entry.getValue().accept(new Command(this, command));
    					break;
    				}
    			}
    		}
